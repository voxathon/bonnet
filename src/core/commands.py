"""Canonical command registry for Bonnet protocol v2 and v3.
...
Protocol v3 (ARTICLE_FEED_PROTOCOL_V3_IMPLEMENTATION_PLAN.md §13) defines a
compact command table that reuses opcodes 0x12-0x1B with new article-feed
semantics. V3_COMMAND_SPECS is the v3 table; COMMAND_SPECS remains the v2
table. The /v3/command endpoint dispatches using V3_COMMAND_SPECS.
"""
...
# ---------------------------------------------------------------------------
# Protocol v2 command table (legacy)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Protocol v3 command table (§13)
# ---------------------------------------------------------------------------

V3_COMMAND_SPECS: dict[int, CommandSpec] = {
    spec.opcode: spec for spec in [
        ...
        CommandSpec(0x70, "PEER_KEY_ROTATE", "write"),
        CommandSpec(0x71, "PEER_KEY_LIST", "read"),
        # New compact article/feed commands (§13.2)
        CommandSpec(0x12, "ARTICLE_PUBLISH", "write"),
        CommandSpec(0x13, "ARTICLE_GET", "read"),
        CommandSpec(0x14, "ARTICLE_LIST", "read"),
        CommandSpec(0x15, "FEED_HEAD", "read", "articles"),
        CommandSpec(0x16, "FEED_EVENTS", "read", "articles"),
        CommandSpec(0x17, "ARTICLE_BODY", "read", "articles"),
        CommandSpec(0x18, "FEED_HEADS", "read", "articles"),
        CommandSpec(0x19, "ARTICLE_SEARCH", "read"),
        CommandSpec(0x1A, "BOARD_SET_STATE", "write"),
        CommandSpec(0x1B, "BAN_STATUS", "read"),
    ]
}

V3_SPECS_BY_NAME: dict[str, CommandSpec] = {spec.name: spec for spec in V3_COMMAND_SPECS.values()}


def get_v3_spec(opcode: int) -> Optional[CommandSpec]:
    return V3_COMMAND_SPECS.get(opcode)


def get_v3_spec_by_name(name: str) -> Optional[CommandSpec]:
    return V3_SPECS_BY_NAME.get(name)
