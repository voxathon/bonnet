...
cdef class Bonnet:
  ...
  quit                  Exit"""
    ...
    cdef str _cmd_register(self, list parts):
        if len(parts) < 2:
    ...
    cdef str _cmd_list_users(self, list parts):
        cdef int offset = 0, limit = 100
    ...
    cdef str _cmd_create_board(self, list parts):
        if len(parts) < 2:
    ...
    cdef str _cmd_list_boards(self):
        request = bytes([0x11])
        response = self.command_handler.handle(request, self.local_conn)
    ...
    cdef str _cmd_close_board(self, list parts):
        if len(parts) < 2:
    ...
    cdef str _cmd_delete_board(self, list parts):
        if len(parts) < 2:
    ...
    cdef str _cmd_list_posts(self, list parts):
        if len(parts) < 2:
    ...
    cdef str _cmd_get_post(self, list parts):
        if len(parts) < 3:
    ...
    async def _interactive_update_post(self, list parts):
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, lambda: self._do_update_post(parts))
    
    cdef str _do_create_post(self, list parts):
        cdef str board
        cdef uint64_t root = 0
        cdef str subject, tags, options, content
        cdef list lines, tags_list
        cdef bytes request, response, content_bytes
        cdef int i
    ...
    cdef str _do_update_post(self, list parts):
        cdef str board
        cdef uint64_t post_num = 0
        cdef bytes get_request, response
        cdef dict post
        cdef list fields
        cdef str new_subject, new_tags, new_options, new_sticky, closed_input
        cdef str new_content_line, content_input
        cdef list content_lines
        cdef int i
        cdef bint is_mod
        cdef bytes request
        cdef object field_type, value
        cdef bytes v_bytes
        cdef list tags_list
    ...
    cdef str _format_create_response(self, bytes data):
        cdef int offset = 1
        cdef uint64_t post_num
        cdef int64_t creation_date, last_modified
        cdef str author, author_registrar, tags, subject, options
        cdef str ts
        from datetime import datetime
    ...
    cdef str _cmd_delete_post(self, list parts):
        if len(parts) < 3:
    ...
    cdef str _cmd_sign_post(self, list parts):
        if len(parts) < 3:
    ...
    cdef str _cmd_query_posts(self, list parts):
        if len(parts) < 2:
        ...
        cdef bytes request = self._build_query_request(board, where, values, orderby, limit)
        response = self.command_handler.handle(request, self.local_conn)
    ...
    cdef str _cmd_demote(self, list parts):
        if len(parts) < 2:
    ...
    cdef str _decode_string(self, bytes data, int offset):
        cdef int length = data[offset]
        return data[offset + 1:offset + 1 + length].decode("utf-8")
    
    cdef str _parse_error(self, bytes response):
        if response[0] == 0x01:
    ...
    cdef str _format_user_list(self, bytes data):
        cdef int offset = 1
    ...
    cdef str _format_board_list(self, bytes data):
        cdef int offset = 1
        cdef int count = struct.unpack(">H", data[offset:offset + 2])[0]
        offset += 2
    ...
    cdef dict _parse_post_data(self, bytes data):
        cdef int offset = 1
        post = {}
    ...
    cdef str _format_post(self, bytes data, str board):
    ...
    cdef bytes _build_signed_payload(self, dict post):
    ...
    cdef bytes _build_query_request(self, str board, str where, list values, str orderby, int limit):
        cdef bytes request = self._encode_string(board)
    ...
    cdef str _format_query_results(self, bytes data):
        cdef int offset = 1
        lines = []
