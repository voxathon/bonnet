...
cdef class BonnetServer:
    ...
    async def handle_connection(self, websocket):
        ...
        try:
            ...
            # Phase 2: Verify signature (with timeout)
            try:
                handshake_frame = await asyncio.wait_for(
                    self._recv_frame(websocket),
                    timeout=30.0
                )
            except asyncio.TimeoutError:
                await self._send_error(websocket, 408, "Handshake timeout", None)
                return

            if len(handshake_frame) != 96:
                await self._send_error(websocket, 400, "Invalid handshake frame length", None)
                return
            ...
            try:
                await self._send_error(websocket, 400, str(e), session)
            except:
                pass
        except Exception as e:
            try:
                await self._send_error(websocket, 500, f"Internal error: {str(e)}", session)
            except:
                pass
    ...
    async def _recv_frame(self, websocket):
        ...
        if isinstance(data, bytes):
            ...
            if len(data) - 4 != length:
                raise ValueError(f"Frame length mismatch: expected {length}, got {len(data) - 4}")
