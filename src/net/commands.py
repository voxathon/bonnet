...
class CommandHandler:
    ...
    def _maybe_queue_remote_feed_sync(self, origin: str, board: str):
        """Lazy federation: if (origin, board) is a subscribed remote feed,
        queue a background sync so a retry will hit. ACLs already checked."""
    ...
    def _cmd_get(self, data: bytes, ctx: CommandContext) -> bytes:
        """GET_USERS_BY_PUBKEY — dual-mode user lookup.

        Request format:
          selector_type:u8
            0x01 = username: u8_len + UTF-8
            0x02 = pubkey:  32 raw bytes

        Response format (same per-user record as LIST_USERS):
          status:u8 = 0x00
          count:u16
          repeated count times:
            username_len:u8 + username
            registrar_len:u8 + registrar
            record_origin_len:u8 + record_origin
            relay_len:u8 + relay
            pubkey_len:u8 (32) + pubkey:32
        """
        try:
            if len(data) < 1:
                return self._build_error(400, "Missing selector type")

            selector_type = data[0]
            idx = 1

            if selector_type == 0x01:
                # Username lookup
                if idx >= len(data):
                    return self._build_error(400, "Missing username length")
                u_len = data[idx]
                idx += 1
                username = data[idx:idx + u_len].decode('utf-8')
                ...
                # Pubkey lookup
                if len(data) - idx < 32:
                    return self._build_error(400, "Pubkey selector requires 32 bytes")
                pubkey = data[idx:idx + 32]
                users = self._ume.get_all_by_publickey(pubkey)
            else:
                return self._build_error(400, f"Invalid selector type {selector_type}")

            if not users:
                return self._build_error(404, "User not found")

            payload = struct.pack('>B', 0x00) + struct.pack('>H', len(users))
            for user in users:
