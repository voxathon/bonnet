...
class CommandHandler:
    ...
    def _cmd_user_registry_head(self, data: bytes, ctx: CommandContext) -> bytes:
        try:
            ...
            if origin != self._config.origin and not ctx.is_unknown:
                return self._build_error(403, "Authentication required for remote registry reads")
    ...
    def _cmd_user_registry_heads(self, data: bytes, ctx: CommandContext) -> bytes:
        try:
            ...
            if not ctx.is_unknown:
                heads = [h for h in heads if h.origin == self._config.origin]
            from core.user_registry import encode_head
