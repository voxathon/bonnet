...
class Connection:
    ...
    @staticmethod
    def client(identity: object, timeout_seconds: int = 30):
        conn = Connection()
    ...
    @staticmethod
    def server(identity: object, websocket: object, engine: object,
               user_callback: object = None, timeout_seconds: int = 30):
        conn = Connection()
    ...
    def can_edit_post(self, author) -> bool:
        return self.user is not None and self.user.username == author

    def can_delete_post(self, author) -> bool:
    ...
    async def send_request(self, cmd: bytes, payload: bytes):
    ...
    async def send_response(self, data: bytes):
        if not self.is_ready:
            raise ConnectionError(500, f"Invalid state: {_state_name(self.state)}")
        if self.session is None:
            raise ConnectionError(500, "No active session")
    ...
    async def _send_error_encrypted(self, code: int, message: str):
        msg_bytes = message.encode('utf-8')
        payload = struct.pack('>BHB', 0x01, code, len(msg_bytes)) + msg_bytes
        encrypted = self.session.encrypt(payload)
        await self._send_frame(encrypted)

    def _create_session(self, our_privkey: bytes, their_pubkey: bytes):
        from core.crypto import EncryptedSession
        return EncryptedSession(our_privkey, their_pubkey)

    async def _handle_user_selection(self, users: list) -> Optional[object]:
