...
# download_and_verify downloads a file from the Ubuntu OpenSSL pool and verifies its SHA256 checksum, exiting with an error if the checksum does not match.
download_and_verify() {
    local filename="$1"
...
# extract_deb extracts the contents of a .deb file into the specified output directory, supporting data.tar.xz and data.tar.zst compression and exiting with status 1 on unsupported compression.
extract_deb() {
    local deb_file="$1"
    local output_dir="$2"
