...
MODULES := orm ame ume crypto config export conman __init
...
pyinstaller: $(BUILD_DIR)/bonnet.so $(BUILD_DIR)/orm.so $(BUILD_DIR)/ame.so $(BUILD_DIR)/ume.so $(BUILD_DIR)/crypto.so $(BUILD_DIR)/config.so $(BUILD_DIR)/export.so $(BUILD_DIR)/conman.so $(BUILD_DIR)/__init.so | $(BIN_DIR)
	cp $(BUILD_DIR)/*.so src/
...
$(BUILD_DIR)/export.c: $(SRC_DIR)/export.pyx | $(BUILD_DIR)
	$(CYTHON) -3 $< -o $@
...
$(BUILD_DIR)/export.so: $(BUILD_DIR)/export.c
	$(CLANG) -shared -fPIC $(CFLAGS) $< -o $@ $(LDFLAGS)
...
$(BIN_DIR)/bonnet: $(BUILD_DIR)/bonnet_embed.c $(BUILD_DIR)/orm.so $(BUILD_DIR)/ame.so $(BUILD_DIR)/ume.so $(BUILD_DIR)/crypto.so $(BUILD_DIR)/config.so $(BUILD_DIR)/export.so $(BUILD_DIR)/conman.so $(BUILD_DIR)/__init.so | $(BIN_DIR)
